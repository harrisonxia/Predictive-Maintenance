...
# train_set = features.join(train_label, on='realdate')
# train_set = train_set.where(train_set['label'].isNotNull())
# train, test = train_set.randomSplit([0.8, 0.2])


# # inputfeature="["+','.join(featuresCol)+"]"
# print(featuresCol)
...
print(res)
# # evaluator = RegressionEvaluator(predictionCol='label')
# # # test=spark.read.csv('test_label_sample.csv')
# # # @functions.udf(returnType=types.DataType())
# # # def chagedate(row):
# # #     return datetime.strptime(row,'%Y/%m/%d')
# # # test.withColumn(test.date)
# # testinput=test.select(featuresCol)
# # prediction=model.transform(testinput)
